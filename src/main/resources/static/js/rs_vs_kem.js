...
document.getElementById('btn-encrypt-both').addEventListener('click', async function() {
    ...
    try {
        const rsaEncryptResponse = await fetch(`${API_BASE}/encrypt-message`, {
            ...
            body: JSON.stringify({ message: message, data: JSON.stringify({ aesKey: state.rsa.aesKey }) })
        });
        ...
        if (rsaEncryptData.success) {
            ...
            const rsaDecryptResponse = await fetch(`${API_BASE}/decrypt-message`, {
                method: 'POST', headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ data: JSON.stringify({ aesKey: state.rsa.aesKey, iv: state.rsa.iv, ciphertext: state.rsa.ciphertext }) })
            });
        ...
        const kemEncryptResponse = await fetch(`${API_BASE}/encrypt-message`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ message: message, data: JSON.stringify({ aesKey: state.kem.sharedSecret }) })
        });
        ...
        if (kemEncryptData.success) {
            ...
            const kemDecryptResponse = await fetch(`${API_BASE}/decrypt-message`, {
                method: 'POST', headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ data: JSON.stringify({ aesKey: state.kem.sharedSecret, iv: state.kem.iv, ciphertext: state.kem.ciphertext }) })
            });
